...
namespace Council.DiscordBot.Commands
{
    ...
    public class RoleRequest : ModuleBase<SocketCommandContext>
    {
        ...
        {
            if (reaction.User.Value.IsBot || !(await cache.GetOrDownloadAsync() is IUserMessage message) || !IsRoleRequestMessage(message))
                return;

            var user = await GetUserWhoReacted(cache, reaction);
            if (user == null)
                return;
